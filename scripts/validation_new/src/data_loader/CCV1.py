...
class CCV1(Dataset):
    ...
    def fill_data(self,max_events):
        ...
        for fi,path in enumerate(tqdm(self.raw_paths)):
            ...
            for array in uproot.iterate(f"{path}:HGCALTBout", ["hit_x", "hit_y", "hit_z", "hit_Edep", "hit_layer", \
                                                               "hit_showerid", "hit_purity", "PrimaryEnergies"], step_size=self.step_size):
                ...
                if counter == 0:
                    ...
                    self.PE = ak.concatenate((self.PE, tmp_PE))
    ...
    def get(self, idx):
        ...
        return Data(
            ...
            PrimaryEnergies = torch.from_numpy(PE).float()
            )
